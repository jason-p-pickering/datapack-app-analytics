...
- Total validations: 
    - COP22: `r validations %>% dplyr::filter(cop_year == 2022) %>% NROW(.)`
    - COP23: `r validations %>% dplyr::filter(cop_year == 2023) %>% NROW(.)`
    - COP24: `r validations %>% dplyr::filter(cop_year == 2024) %>% NROW(.)`
- Total downloads: 
    - COP22: `r downloads %>% dplyr::filter(cop_year == 2022) %>% NROW(.)`
    - COP23: `r downloads %>% dplyr::filter(cop_year == 2023) %>% NROW(.)`
    - COP24: `r downloads %>% dplyr::filter(cop_year == 2024) %>% NROW(.)`
...
validations %>% 
  ...
  dplyr::filter(cop_year %in% c(2023,2024)) %>% 
...
knitr::kable(validations_by_country_total) %>% 
...
validations %>% 
  ...
  dplyr::group_by(Week, cop_year) %>% 
  dplyr::summarise(Validations = dplyr::n())

ggplot(validations_by_date_total,aes(x = Week, y= Validations)) + geom_col() + facet_grid(cop_year ~ .) 
...
```{r downloads_summary_table, eval=TRUE, echo=FALSE, message=FALSE}
downloads_summary_table <- downloads_by_day_type %>% dplyr::group_by(Type,Year) %>% dplyr::summarise(Downloads = sum(Downloads), .groups="drop")  %>% dplyr::mutate(Year = factor(Year))%>% dplyr::arrange(Type, Year) %>%  tidyr::spread(key = Year, value = Downloads, fill = "")
knitr::kable(downloads_summary_table, align = "lrrr")
```
...
s3<-paws::s3()
s3_files <- s3$list_objects_v2(Bucket=Sys.getenv("AWS_S3_BUCKET"),Prefix="validation_error_summary/cop24/") 
...
while(is_truncated) {
  s3_files_next <- s3$list_objects_v2(Bucket=Sys.getenv("AWS_S3_BUCKET"),
                                      Prefix="validation_error_summary/cop24/",
...
countries <- files_df[,3] %>% stringr::str_remove("summary/cop24/")
dates <- files_df[,4]
...
dir.create(paste0(tmpdir,"/","validation_error_summary/cop24"))
...
file_prefix<-paste0(tmpdir,"/validation_error_summary/cop24/")
...
kable(issue_summary,caption=paste("Validation summary ( n =",length(unique(d$country_name)),")"))

...
- Logins: `r dplyr::filter(logins, event_type == "LOGIN") %>% nrow()`
- Failed logns: `r dplyr::filter(logins, event_type == "LOGIN_FAILED") %>% nrow()`
...
validations %>% 
  dplyr::select(Country=datapack_name,Type = message) %>% 
...
downloads %>% 
  dplyr::select(Date=event_date,Type = message) %>% 
  dplyr::group_by(Date,Type,) %>% 
  dplyr::summarise(Downloads = dplyr::n())

ggplot(downloads_by_day_type,aes(x = Date, y= Downloads, fill=Type)) + geom_bar(position="stack", stat="identity")
...
s3<-paws::s3()
s3_files <- s3$list_objects_v2(Bucket=Sys.getenv("AWS_S3_BUCKET"),Prefix="validation_error_summary/cop22/") 
...
while(is_truncated) {
  s3_files_next <- s3$list_objects_v2(Bucket=Sys.getenv("AWS_S3_BUCKET"),Prefix="app_analytics/cop22/",ContinuationToken = coninutation_token)
...
countries <- files_df[,3] %>% stringr::str_remove("summary/cop22/")
dates <- files_df[,4]
...
dir.create(paste0(tmpdir,"/","validation_error_summary/cop22"))
...
file_prefix<-paste0(tmpdir,"/validation_error_summary/cop22/")
